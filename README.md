...
| 🍎 **macOS Apple Silicon** (M1–M4) | `GameOfTheAmazons-macOS-Silicon.zip` | Unzip, drag `GameOfTheAmazons.app` to Applications. **⚠️ CRITICAL**: Safari auto-extracts zips on download, which re-applies the macOS quarantine flag. **After moving to Applications, you MUST run this in Terminal:** `xattr -cr /Applications/GameOfTheAmazons.app` (No output = success). Then launch normally. If you see "damaged or incomplete", the quarantine flag wasn't cleared — run the command above. |
